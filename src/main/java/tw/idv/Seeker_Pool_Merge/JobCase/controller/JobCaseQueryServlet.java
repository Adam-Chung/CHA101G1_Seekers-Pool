...
public class JobCaseQueryServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
		 ...
		 * 開始 查詢單一資料 轉交
		 ****************************************/
		if ("selectByNo".equals(action)) {
			System.out.println("進入 selectByNo");
			List<String> errorMsgs = new LinkedList<String>();
			req.setAttribute("errorMsgs", errorMsgs);

			/** 1.接收請求參數 - 輸入格式的錯誤處理 **/
			String str_jcNo = req.getParameter("jcNo");
			...
			if (!errorMsgs.isEmpty()) {
			    // Forward to the appropriate error handling mechanism and display the error messages
...
//			System.out.println(action);
//			List<ComOrderQueryVO> list=JcDao.selectOrderCase();
...
//			
//			/** 1.接收請求參數 - 輸入格式的錯誤處理 **/
//			String str_joNo = req.getParameter("joNo");
//			System.out.println(str_joNo);
//			
//			if (str_joNo == null || str_joNo.trim().isEmpty()) {
//				errorMsgs.add("請輸入方案編號");
//			} else if (!str_joNo.matches("\\d+")) {
//				errorMsgs.add("方案編號格式不正確(ex:方案一)");
//			}
//			
//			if (!errorMsgs.isEmpty()) {
...
//			
//			// Convert the validated jcNo to an Integer
//			Integer joNo = Integer.valueOf(str_joNo);
//			
//			 List<JobCaseVO> jobCases = JcDao.selectOrderCase();
//			 List<JobCaseVO> selectedJobCases = new ArrayList<>();
//
//			    for (JobCaseVO jobCase : jobCases) {
//			        if (jobCase.getJoNo().equals(joNo)) {
//			            selectedJobCases.add(jobCase);
//			        }
//			    }
//			
//			// Perform the data query using jcNo
//			List<JobCaseVO> jc = JcDao.selectOrderCase();
//			writePojo2Json(resp, jc);
//			
//		}
